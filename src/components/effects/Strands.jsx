...
export default function Strands({
  ...
  // Respect prefers-reduced-motion: tone down animation speed, glow pulsing
  // and intensity. Keep the visual present (don't hide it) but make it calmer.
  const reduced =
    typeof window !== "undefined" &&
    window.matchMedia?.("(prefers-reduced-motion: reduce)").matches;
  propsRef.current = {
    colors,
    count: reduced ? Math.min(count, 3) : count,
    speed: reduced ? speed * 0.2 : speed,
    amplitude: reduced ? amplitude * 0.6 : amplitude,
    waviness: reduced ? waviness * 0.7 : waviness,
    thickness,
    glow: reduced ? glow * 0.65 : glow,
    taper,
    spread,
    hueShift,
    intensity: reduced ? intensity * 0.6 : intensity,
    saturation,
    opacity: reduced ? opacity * 0.8 : opacity,
    scale,
    glass,
    refraction,
    dispersion,
