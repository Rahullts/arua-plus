...
// Quality presets cap pixel ratio (and thus shader fragment cost).
// 'auto' starts at high and steps down if FPS drops below target.
const QUALITY_DPR = { low: 0.75, medium: 1, high: 1.5, ultra: 2 };
...
export const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    const qCap = QUALITY_DPR[quality] ?? QUALITY_DPR.high;
    const hardCap = Math.min(maxDpr ?? 2, 2);
    // Explicit `dpr` prop wins; otherwise clamp device DPR by quality + hardCap.
    const initialDpr = dpr ?? Math.min(deviceDpr, qCap, hardCap);
    baseDprRef.current = initialDpr;
    ...
    let raf = 0;
    // FPS-cap throttle: target ms-per-frame; 0 = uncapped.
    const frameInterval = fpsCap > 0 ? 1000 / fpsCap : 0;
    let lastFrameMs = 0;
    // Auto-downgrade thresholds (avg ms/frame). 22ms ≈ 45fps.
    const DOWNGRADE_MS = 22;
    const downgradeSteps = [2, 1.5, 1, 0.75, 0.5];
    const animate = () => {
      raf = requestAnimationFrame(animate);
      if (pausedRef.current || !inViewRef.current) return;
      if (frameInterval > 0) {
        const nowMs = performance.now();
        if (nowMs - lastFrameMs < frameInterval) return;
        lastFrameMs = nowMs;
      }
      ...
      // Auto-quality: every ~1s of sustained low FPS, step DPR down one notch.
      if (quality === "auto" && hasFadedRef.current && emaDtRef.current > DOWNGRADE_MS) {
        const nowMs = performance.now();
        if (nowMs - lastFpsCheckRef.current > 1000) {
          lastFpsCheckRef.current = nowMs;
          const cur = currentDprRef.current;
          const idx = downgradeSteps.findIndex((s) => s < cur);
          if (idx !== -1) {
            currentDprRef.current = downgradeSteps[idx];
